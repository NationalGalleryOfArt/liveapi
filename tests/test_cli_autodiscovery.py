...
class TestFirstRunSetup:
    ...
    def test_setup_first_run_single_spec(
        self, mock_generator_class, temp_dir, sample_openapi_spec
    ):
        ...
        mock_generator.get_default_output_path.return_value = (
            "implementations/user_service.py"
        )
    ...
    def test_setup_first_run_multiple_specs(
        self,
        mock_gitignore,
        mock_organize,
        mock_create_main,
        ...
        mock_generator = mock_generator_class.return_value
        mock_generator.get_default_output_path.return_value = (
            "implementations/test_service.py"
        )
        mock_generator.generate_scaffold.return_value = True
...
class TestIncrementalMode:
    ...
    @patch("automatic.cli.ScaffoldGenerator")
    def test_find_missing_implementations_with_missing(
        self, mock_generator_class, temp_dir, sample_openapi_spec
    ):
    ...
    @patch("automatic.cli.ScaffoldGenerator")
    def test_find_missing_implementations_all_exist(
        self, mock_generator_class, temp_dir, sample_openapi_spec
    ):
...
class TestAutoDiscoveryModes:
    ...
    def test_auto_discover_first_run_mode(
        self, mock_find_specs, mock_setup, temp_dir, sample_openapi_spec
    ):
    ...
    def test_auto_discover_incremental_mode(
        self, mock_find_missing, mock_add_missing, temp_dir
    ):
    ...
    @patch("automatic.cli.find_missing_implementations")
    def test_auto_discover_incremental_mode_no_missing(
        self, mock_find_missing, temp_dir, capsys
    ):
        ...
        try:
            ...
            assert (
                "All specifications have corresponding implementations" in captured.out
            )
...
class TestMainPyGeneration:
    ...
    def test_main_py_uses_versioned_spec_paths(self, temp_dir):
        ...
        # Create minimal valid OpenAPI content
        sample_spec = {
            "openapi": "3.0.0",
            "info": {"title": "Test API", "version": "1.0.0"},
            "paths": {
        ...
        try:
            ...
            assert (
                "specifications/users_v1.yaml" in main_content
            ), "Should reference users_v1.yaml"
            assert (
                "specifications/orders_v1.yaml" in main_content
            ), "Should reference orders_v1.yaml"

            # Verify original names are NOT used
            assert (
                "specifications/users.yaml" not in main_content
            ), "Should not reference unversioned users.yaml"
            assert (
                "specifications/orders.yaml" not in main_content
            ), "Should not reference unversioned orders.yaml"

            # Verify comments also use versioned names
            assert (
                "# users_v1.yaml" in main_content
            ), "Comments should reference versioned files"
            assert (
                "# orders_v1.yaml" in main_content
            ), "Comments should reference versioned files"
