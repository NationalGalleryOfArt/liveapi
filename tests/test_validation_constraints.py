...
class TestJSONSchemaValidationConstraints:
    ...
    def test_pydantic_model_generation_with_constraints(self, test_spec):
        ...
        ProductModel = generator.generate_model_from_schema(
            test_spec["components"]["schemas"]["Product"], "Product"
        )
        ...
        with pytest.raises(Exception) as exc_info:
            ProductModel(
                name="A" * 101, price=19.99, category="electronics"  # Too long
            )
        ...
        assert (
            "not a valid enumeration member" in error_msg
            or "input should be" in error_msg
            or "literal_error" in error_msg
            or "unexpected value" in error_msg
        )

        # Test minItems constraint
        with pytest.raises(Exception) as exc_info:
            ProductModel(
                name="Test", price=19.99, category="electronics", tags=[]  # Empty array
            )
...
class TestHTTP422ErrorResponses:
    ...
    def test_openapi_spec_has_correct_422_schema(self, test_client):
        ...
        validation_error_schema = openapi_spec["components"]["schemas"][
            "ValidationError"
        ]
        ...
        schema_ref = error_response["content"]["application/problem+json"]["schema"][
            "$ref"
        ]
