...
from src.liveapi.implementation.exceptions import NotFoundError, ValidationError, ConflictError
...
class TestCRUDHandlers:
    ...
    async def test_update_full_success(self):
        ...
        # Create a resource first
        data = {"id": "update-test", "name": "Original", "email": "original@example.com", "age": 25}
    ...
    async def test_update_partial_success(self):
        ...
        # Create a resource first
        data = {"id": "patch-test", "name": "Original", "email": "original@example.com", "age": 25}
        created = await self.handlers.create(data)
    ...
    async def test_list_with_resources(self):
        ...
        for i in range(5):
            data = {"id": f"user-{i}", "name": f"User {i}", "email": f"user{i}@example.com"}
            await self.handlers.create(data)
    ...
    async def test_list_with_filters(self):
        ...
        await self.handlers.create({"id": "user-1", "name": "Alice", "email": "alice@example.com", "age": 25})
        await self.handlers.create({"id": "user-2", "name": "Bob", "email": "bob@example.com", "age": 30})
        await self.handlers.create({"id": "user-3", "name": "Charlie", "email": "charlie@example.com", "age": 35})
...
class TestCreateCrudRouter:
    ...
    def test_router_response_models(self):
        ...
        with patch('fastapi.APIRouter') as mock_router_class:
            ...
            individual_get = [call for call in get_calls if "resource_id" in call[0][0]][0]
...
class TestCRUDHandlersIntegration:
    ...
    async def test_full_crud_workflow(self):
        ...
        # Create
        create_data = {"name": "Integration Test", "email": "integration@example.com", "age": 28}
        ...
        # Update
        update_data = {"name": "Updated Integration", "email": "updated@example.com", "age": 30}
