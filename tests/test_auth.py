...
def app_no_auth(test_spec_content, tmp_path):
    ...
    # Create app
    app = create_app(spec_path=str(spec_file), implementation=AuthTestImplementation())
...
def test_bearer_request_with_valid_token(app_with_bearer_auth):
    ...
    response = client.get(
        "/users", headers={"Authorization": "Bearer bearer-token-123"}
    )
...
def test_bearer_request_with_second_valid_token(app_with_bearer_auth):
    ...
    response = client.get(
        "/users", headers={"Authorization": "Bearer bearer-token-456"}
    )
